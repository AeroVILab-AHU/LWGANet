...
class LWGA(nn.Module):
    def __init__(self,
        ...
        self.PA = P_Conv(self.dim_learn, act_layer) # P_Conv is point attention
        self.LA = Conv(self.dim_learn, act_layer)       # LA is local attention
        self.MRA = MRA(self.dim_learn, att_kernel)      # MRA is medium range attention
        if stage == 2:
            self.D_GA = D_GA(self.dim_untouched)        # GA is global attention
        elif stage == 3:
