...
def LWGANet_L0_1242_e32_k11_GELU(num_classes=2, embed_dim=32, pretrained=None, **kwargs):
...
def LWGANet_L1_1242_e64_k11_GELU_drop01(num_classes=2, embed_dim=64, pretrained=None, **kwargs):
    model = LWGANet(in_chans=3,
                    num_classes=num_classes,
                    embed_dim=embed_dim,
                    depths=(1, 2, 4, 2),
                    ...
                    **kwargs)
    if pretrained == True:
        checkpoint = torch.load(pretrained, map_location=torch.device('cuda:0'))
        _state_dict = checkpoint['model']
        state_dict = _state_dict
        missing_keys, unexpected_keys = model.load_state_dict(state_dict, False)
...
def LWGANet_L2_1442_e96_k11_ReLU_drop01(num_classes=2, embed_dim=96, pretrained=None, **kwargs):
    model = LWGANet(in_chans=3,
                    num_classes=num_classes,
                    embed_dim=embed_dim,
                    depths=(1, 4, 4, 2),
